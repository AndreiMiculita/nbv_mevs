...
class ModelNet10DepthDataset(Dataset):
    def __init__(self, transforms=None, train=True):
        super().__init__()

        self.transforms = transforms

        self._cache = os.path.join(DATA_DIR, "depth-dataset_cache")

        if not os.path.exists(self._cache):
            self.folder = "depth-dataset"
...
if __name__ == "__main__":
    ...
    dset = ModelNet10DepthDataset(transforms=transforms, train=True)
