...
ckpt_dir = Path(__file__).parent.parent / 'data/ckpt_files/'

image_dataset_dir = Path('/home/andrei/PycharmProjects/Pointnet2_PyTorch/data/image-dataset-mnet10-10views/')
...
def main():
    for ckpt_file in ckpt_dir.iterdir():
        ...
        for filename in test_set_filenames:
            ...
            test_set_filenames_grouped[(class_name, object_id)].append((filename, float(theta), float(phi), int(vc)))
